...
function writeUserData(soat, mn, som, blk ) {
  ...
  set(ref(db, 'maxsulotchqim'+blk+'/'+ soat), {
    narx: som+'000',
    joy:mn,
    vaqat:soat,
  ...
  const starCountRef = ref(db, 'maxsulotchqimoylar');
  const dt=new Date();
  ...
  let tek=''
    if (data) {
        ...
        if (tek[tek.length-1].yilkun!=ky || !data) {
            set(ref(db, 'maxsulotchqimoylar/'+ ky), {
                yilkun:ky
              });
          }
    }else{
...
function MaxsulotChqim() {
    ...
    const nv=useNavigate();
       ...
       useEffect(()=>{
        ...
        const dt=new Date();
        const ky=dt.getMonth()+1+''+dt.getFullYear();
        const starCountRef = ref(db, 'maxsulotchqim'+ky);
        onValue(starCountRef, (snapshot) => {
        const data = snapshot.val();    
        if (data) {
