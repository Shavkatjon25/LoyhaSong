...
function writeUserData(soat, mn, som, blk, tel ) {

  set(ref(db, 'zakas'+blk+'/'+ soat), {
    narx: som+'000',
    joy:mn,
    vaqat:soat,
  ...
  const starCountRef = ref(db, 'zakasoylar');
  const dt=new Date();
...
function Zakas() {
    ...
    const nv=useNavigate();
       ...
       useEffect(()=>{
        ...
        const dt=new Date();
        const ky=dt.getMonth()+1+''+dt.getFullYear();
        const starCountRef = ref(db, 'zakas'+ky);
        onValue(starCountRef, (snapshot) => {
        const data = snapshot.val();    
        if (data) {
    ...
    function Royhat(m){
        ...
        return(
            ...
            <div>
                ...
                <h3 className="font-thin">Tel: {m.tel}</h3>
            </div>
            <div>
            <h2>{m.name}</h2>
    ...
    function Hendl(){
        ...
        writeUserData(soat, mn, som, ky, tel);
        setMd(!md)
        setMn('');
        setSom('');
        setTel('')
  ...
  return (
    ...
    <div className={`w-full h-[100vh] fixed bg-[#0000005b] top-0 justify-center z-30 pt-5 ${md ? 'flex ht' : 'hidden'}`} onClick={()=>setMd(!md)}>
        <div className="w-[340px] h-[320px] flex flex-col gap-8 rounded-2xl bg-slate-500 px-5 py-10" onClick={e=>e.stopPropagation()}>
            ...
            <input type="number" value={tel} onChange={e=>setTel(e.target.value)} placeholder="Tel"  className="h-10 rounded-md p-5 text-3xl" />
